<template>
  <div v-if="show">
    <el-form label-position="top">
      ...
      <!-- 标题 -->
      <el-form-item
        v-if="data.type !== 'grid' && data.type !== 'divider'"
      ...
      >
        <el-checkbox v-model="data.options.labelWidthDisabled">{{
        ...
        <el-input-number
          ...
          :min="0"
          :step="10"
          :disabled="!data.options.labelWidthDisabled"
        />
      </el-form-item>
      <el-form-item
        v-if="
      ...
      >
        <el-switch v-model="data.options.labelWidthStatus" />
      </el-form-item>
      <el-form-item
      ...
      >
        ...
        >
          ...
          <el-radio-button :label="99">{{
            $t('fm.config.widget.requestMethod')
          }}</el-radio-button>
        </el-radio-group>
          ...
          <div class="form-request-method">
            <div>
              ...
              <div class="form-request-method-content">
                <el-input
                  v-model="data.options.requestMethod.url"
                  :placeholder="$t('fm.config.widget.enterUrl')"
                  size="mini"
                />
              </div>
            </div>
            <div>
              <div class="form-request-method-title">
                {{ $t('fm.config.widget.requestMethod') }}
              </div>
              <div class="form-request-method-content">
                <el-radio-group
                  v-model="data.options.requestMethod.method"
                  size="mini"
                >
            ...
            <div>
              <div class="form-request-method-title" style="margin-bottom: 6px">
                {{ $t('fm.config.widget.requestParams') }}
              </div>
              <div class="form-request-method-content">
                <el-input
                  ...
                  type="textarea"
                  :rows="2"
                  :placeholder="$t('fm.config.widget.enterContent')"
                />
              </div>
            </div>
            <div>
              <div class="form-request-method-title" style="margin-bottom: 6px">
                {{ $t('fm.config.widget.requesetHeader') }}
              </div>
              <div class="form-request-method-content">
                <el-input
            ...
            <div>
              <div class="form-request-method-title" style="margin-bottom: 6px">
                {{ $t('fm.config.widget.timeOutPeriod') }}
              </div>
              <div class="form-request-method-content">
                <el-input
            ...
            <div>
              <div class="form-request-method-title" style="margin-bottom: 6px">
                {{ $t('fm.config.widget.returnField') }}
              </div>
              <div class="form-request-method-content">
                <el-input
            ...
            <div>
              <div class="form-request-method-title" style="margin-bottom: 6px">
                {{ $t('fm.config.widget.fieldMapping') }}
              </div>
              <div class="form-request-method-content">
                <el-input
        ...
        <template v-else>
          ...
          >
            <el-button
              type="text"
              :style="data.type === 'cascader' ? { 'font-size': '13px' } : {}"
              @click="
                data.type === 'cascader'
      ...
      <template v-if="data.type === 'text'">
        <el-form-item :label="$t('fm.config.widget.textContent')">
          <el-input
            v-model="data.options.defaultValue"
            :placeholder="$t('fm.config.widget.enterTextContent')"
          />
        </el-form-item>
        <el-form-item :label="$t('fm.config.widget.fontSize')">
          <el-input
            v-model="data.options.font_size"
            :placeholder="$t('fm.config.widget.enterFontSize')"
          />
        </el-form-item>
        <el-form-item :label="$t('fm.config.widget.fontColor')">
          <el-input
            v-model="data.options.font_color"
            :placeholder="$t('fm.config.widget.enterFontColor')"
          />
        </el-form-item>
        <el-form-item :label="$t('fm.config.widget.setBold')">
          <el-input
            v-model="data.options.font_weight"
            :placeholder="$t('fm.config.widget.enterSetBold')"
          />
        </el-form-item>
        <el-form-item :label="$t('fm.config.widget.fontAttribute')">
          <el-input
            v-model="data.options.font_family"
            :placeholder="$t('fm.config.widget.enterFontAttribute')"
          />
        </el-form-item>
      </template>
      <!-- 分割符 -->
      <template v-if="data.type === 'divider'">
        ...
        <el-form-item :label="$t('fm.config.widget.textPosition')">
        ...
        <el-form-item :label="$t('fm.config.widget.dividingLineDirection')">
          <el-radio-group v-model="data.options.direction">
            <el-radio-button label="horizontal">{{
              $t('fm.config.widget.horizontal')
            }}</el-radio-button>
            <el-radio-button label="vertical">{{
              $t('fm.config.widget.vertical')
            }}</el-radio-button>
          </el-radio-group>
        </el-form-item>
        <el-form-item :label="$t('fm.config.widget.fontSize')">
      ...
      <template v-if="data.type !== 'grid'">
        ...
        <el-form-item :label="$t('fm.config.widget.validate')">
          ...
          >
            ...
            <el-option
              value="boolean"
              :label="$t('fm.config.widget.boolean')"
            />
            <el-option
              value="integer"
      ...
      <el-form-item
        v-if="Object.keys(data.options).indexOf('displayVerify') >= 0"
        :label="$t('fm.config.widget.displayVerify')"
      >
        <el-radio-group v-model="data.options.displayVerify.type">
          <el-radio label="hide">{{
            $t('fm.config.widget.noVerify')
          }}</el-radio>
          <el-radio label="and">{{ $t('fm.config.widget.and') }}</el-radio>
          <el-radio label="or">{{ $t('fm.config.widget.or') }}</el-radio>
        </el-radio-group>
          ...
          <template v-for="(item, index) in data.options.displayVerify.list">
            <div :key="item.model">
              ...
              <el-button
                ...
                @click="delDisplayVerifiy(index)"
                >{{ $t('delete') }}</el-button
              >
              <hr
                v-if="data.options.displayVerify.list.length > 1"
                style="background-color: #dcdfe6; border: none; height: 1px"
              />
            </div>
          </template>
          <el-button
    ...
    <el-dialog
      :title="$t('fm.config.widget.promot')"
    ...
    >
      <div>
        <el-form ref="addTreeData" :model="addTreeData" label-width="80px">
          <el-form-item
            ...
            :rules="{
              required: true,
              message: $t('fm.config.widget.labelRequired'),
              trigger: 'blur'
            }"
          >
          ...
          <el-form-item
            ...
            :rules="{
              required: true,
              message: $t('fm.config.widget.valueRequired'),
              trigger: 'blur'
            }"
          >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="cascaderDialog = false">{{
          $t('fm.actions.cancel')
        }}</el-button>
        <el-button
          type="primary"
  ...
  methods: {
    addDisplayVerifiy() {
      this.data.options.displayVerify.list.push({
        ...
        value: this.$t('fm.config.widget.filedValue')
      })
    },
