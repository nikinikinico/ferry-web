        ...
        <div class="text item">
          ...
          <div v-else>
            <el-alert
              show-icon
              :title="
                $t('未找到当前工单流程信息，请确认当前工单绑定的流程是否存在。')
              "
              type="warning"
            />
          </div>
      ...
      <el-card class="box-card" style="margin-top: 15px">
        ...
        <div class="text item">
          <el-table :data="circulationHistoryList" border style="width: 100%">
            <el-table-column prop="state" :label="$t('节点')" />
            <el-table-column prop="circulation" :label="$t('流转')" />
            <el-table-column prop="processor" :label="$t('处理人')" />
            <el-table-column prop="create_time" :label="$t('处理时间')" />
  ...
  methods: {
    ...
    getAlertMessage() {
      if (this.processStructureValue.workOrder.is_end === 1) {
        this.alertMessage = this.$t('当前工单已结束。')
      }
    },
    activeOrderActive() {
      var jsonData = [
