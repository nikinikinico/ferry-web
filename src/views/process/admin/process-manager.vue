<template>
  <div class="app-container">
    <el-card class="box-card">
      ...
      <el-dialog
        :title="
          dialogProcessVisibleName === 1 ? $t('新建流程') : $t('编辑流程')
        "
        :visible.sync="open"
        :fullscreen="true"
        style="margin-top: 0"
      >
        <div class="tpl-create-content">
          ...
          >
            ...
            <el-row>
              <el-col :span="12">
                <el-form-item :label="$t('分类')" prop="classify">
                  <el-select
              ...
              <el-col :span="12">
                <el-form-item :label="$t('模版')" prop="tpls">
                  <el-select
              ...
              <el-col :span="12">
                <el-form-item :label="$t('通知')">
                  <el-select
                    ...
                    filterable
                    clearable
                    :placeholder="$t('请选择流程任务')"
                    style="width: 100%"
                  >
                    <el-option :label="$t('邮件')" :value="1" />
                  </el-select>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item :label="$t('任务')">
                  <el-select
            ...
            <el-form-item :label="$t('描述')" prop="remarks">
              <el-input
                ...
                :placeholder="$t('请输入流程描述')"
                type="textarea"
                :autosize="{ minRows: 2, maxRows: 4 }"
                style="width: 100%"
              />
            </el-form-item>
            <el-form-item :label="$t('流程')" prop="structure">
    ...
    return {
      ...
      rules: {
        ...
        tpls: [
          {
            required: true,
            message: this.$t('请选择模版'),
            trigger: 'blur, change'
          }
        ],
        structure: [
          { required: true, message: this.$t('请设计流程'), trigger: 'blur' }
        ],
        remarks: [
          {
            required: true,
            message: this.$t('请输入流程描述'),
            trigger: 'blur'
          }
        ]
      }
    ...
    verifyProcess(structureValue) {
      for (var r of structureValue.nodes) {
        if (r.sort === undefined || r.sort === null || r.sort === '') {
          return this.$t('流程节点顺序不能为空')
        } else if (
          r.label === undefined ||
          r.label === null ||
          r.label === ''
        ) {
          return this.$t('流程节点标题不能为空')
        }
        if (r.clazz === 'userTask' || r.clazz === 'receiveTask') {
          if (
            ...
            return this.$t('审批节点或处理节点的处理人类型不能为空')
          } else if (
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (
            ...
            this.$message.error(this.t('没有流程数据，请完善流程'))
          }
        }
      })
    },
